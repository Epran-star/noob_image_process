...
img_masking = np.zeros((len(gray),len(gray[0])),dtype='uint8')

for i in range(1,len(gray)-1):
    for ii in range(1,len(gray[0])-1):
        pixel = gray[i][ii]+abs(gray[i][ii]-smooth_kernal(gray,i,ii,1))
        if pixel>255:
            img_masking[i][ii] = 255
        else:
            img_masking[i][ii] = pixel
        

cv2.imwrite('masking.jpg',img_masking)
